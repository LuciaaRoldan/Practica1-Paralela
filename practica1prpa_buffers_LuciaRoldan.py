...
def producer(buffer,non_empty,empty,M,mutex):
...
def consumer(buffers, sol,non_empty_list,empty_list,lectura,M,mutex):
...
def main():
     ...
     prodlst = [ Process(target=producer,
                        name=f'prod_{i}',
                        args=(buffers[i],non_empty_list[i], empty_list[i],M,mutex))
                for i in range(NPROD) ]
     ...
     consumidor = Process(target=consumer,args=(buffers,solucion,non_empty_list, empty_list, posiciones_lectura_buffer,M,mutex))
